...
def IOU(result, lable):
    ...
    return [np.sum(img_sum >= 500), np.sum(img_sum >= 200)]
...
def evaluate():
    lable_path = "E:\\Experiment data\\GFtrainin\\test\\overall\\water\\"
    result_path ="E:\\Experiment data\\GFtrainin\\test\\resultDenseMSFCN\\water\\"
    # resultMSUNet3\\1024\\3input\\
    ...
    for imgname in imgsname:
        ...
        # # #
        # imgdata_gt = zhankai2(imgdata_gt)
        # imgdata = zhankai2(imgdata)

        imgdata_gt = cloud(imgdata_gt)
        imgdata = cloud(imgdata)

        # imgdata_gt = shadow(imgdata_gt)
        # imgdata = shadow(imgdata)
